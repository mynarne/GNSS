...
<br>
- 소셜 로그인 서비스 추가 (네이버, 카카오)

- 소셜 로그인으로 접속하기 전에 미리 가입된 정보가 있으면 소셜 로그인으로 계정 통합 진행 (네이버만 해당)

#### 2. 관리자 기능 일부 추가
- 비매너 유저 제재 처리 로직 구현 (테스트 필요)

- 일시정지, 정지, 정지 해제의 제재 로직

---

> 26.05.12  금일 완료된 작업
#### 1. 회원가입 로직 최종 작업
- 이메일 인증번호 3분 제약시간 웹 시각화

- 이메일 로그인 방식이 아닌 아이디 탭 추가하여 아이디 기반 로그인으로 수정

#### 2. 회원정보 수정
- 로그인 했을 때 : 마이페이지에서 정보 수정 시 패스워드 입력하여 한 번 더 검증

- 계정을 잊었을 때 : 아이디 찾기 / 패스워드 찾기 기능 나누고 이메일 인증번호를 전송하여 한 번 더 타인이 아닌 실제 유저 검증

#### 3. 디자인 소폭 수정
- 블록이 위로 올라오는 호버 효과 삭제
